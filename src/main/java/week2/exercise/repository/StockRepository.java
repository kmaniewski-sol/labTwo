...
public interface StockRepository extends JpaRepository<Stock, Serializable> {
    ...
    @Query("SELECT price FROM Stock s WHERE s.symbol = :symbol AND MONTH(DATE(s.date)) = MONTH(:date) ORDER BY s.date DESC")
    ...
    @Query("SELECT MAX(price), MIN(price), SUM(volume) FROM Stock s WHERE s.symbol = :symbol AND MONTH(DATE(s.date)) = MONTH(:date)")
